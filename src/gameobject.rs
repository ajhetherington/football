...
pub fn arange_checks(objects: &mut Vec<VisiblePlayer>, radius: f32) {
    objects.sort_by(|a, b| match a.object.pos.x.partial_cmp(&b.object.pos.x) {
        Some(val) => val,
...
pub fn inelastic_check_player_collisions(object_a: &mut GameObject, object_b: &mut GameObject) {
    let squared =
        (object_a.pos.x - object_b.pos.x).powf(2.0) + (object_a.pos.y - object_b.pos.y).powf(2.0);
    if squared >= (object_a.radius + object_b.radius).powf(2.0) {
        return;
    }

    let total_mass = object_a.mass + object_b.mass;
    let a_x_vel = (((object_a.mass - PLAYER_ELASTICITY * object_b.mass) * object_a.x_velocity)
        + (1.0 + PLAYER_ELASTICITY) * object_b.mass * object_b.x_velocity)
        / (total_mass);
    let a_y_vel = (((object_a.mass - PLAYER_ELASTICITY * object_b.mass) * object_a.y_velocity)
        + (1.0 + PLAYER_ELASTICITY) * object_b.mass * object_b.y_velocity)
        / (total_mass);
    let b_x_vel = (((object_b.mass - PLAYER_ELASTICITY * object_a.mass) * object_b.x_velocity)
        + (1.0 + PLAYER_ELASTICITY) * object_a.mass * object_a.x_velocity)
        / (total_mass);
    let b_y_vel = (((object_b.mass - PLAYER_ELASTICITY * object_a.mass) * object_b.y_velocity)
        + (1.0 + PLAYER_ELASTICITY) * object_a.mass * object_a.y_velocity)
        / (total_mass);
